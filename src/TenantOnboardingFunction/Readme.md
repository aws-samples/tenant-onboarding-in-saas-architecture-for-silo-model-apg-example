...
WORKDIR /src
COPY ["TenantOnboardingFunction.csproj", "TenantOnboardingFunction/"]
RUN dotnet restore "TenantOnboardingFunction/TenantOnboardingFunction.csproj"

WORKDIR "/src/TenantOnboardingFunction"
COPY . .
RUN dotnet build "TenantOnboardingFunction.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "TenantOnboardingFunction.csproj" \
...
Execute unit tests
```
cd "TenantOnboardingFunction/test/TenantOnboardingFunction.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "TenantOnboardingFunction/src/TenantOnboardingFunction"
