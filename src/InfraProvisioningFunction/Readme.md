...
WORKDIR /src
COPY ["InfraProvisioningFunction.csproj", "InfraProvisioningFunction/"]
RUN dotnet restore "InfraProvisioningFunction/InfraProvisioningFunction.csproj"

WORKDIR "/src/InfraProvisioningFunction"
COPY . .
RUN dotnet build "InfraProvisioningFunction.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "InfraProvisioningFunction.csproj" \
...
Execute unit tests
```
cd "InfraProvisioningFunction/test/InfraProvisioningFunction.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "InfraProvisioningFunction/src/InfraProvisioningFunction"
